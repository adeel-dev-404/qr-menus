...
<div style="max-width:560px;">
    <div class="form-card">
        <h3 style="font-size:16px; font-weight:700; color:#fff; margin:0 0 20px;">Edit Category</h3>
        ...
        <form method="POST" action="{{ route('dashboard.categories.update', $category) }}" enctype="multipart/form-data">
            ...
            <div style="margin-bottom:16px;">
                ...
                <input type="text" name="name" value="{{ old('name', $category->name) }}" placeholder="e.g. Burgers"
                ...
                @foreach($languages as $lang)
                    ...
                    <div style="margin-bottom:10px;">
                        ...
                        <input type="text" name="translations[{{ $lang }}][name]"
                               value="{{ old('translations.' . $lang . '.name', $category->translations->where('locale', $lang)->where('field', 'name')->first()?->value) }}"
            ...
            <div style="margin-bottom:16px;">
                ...
                <div style="border:1px dashed #2a2a2a; border-radius:8px; padding:20px; text-align:center; cursor:pointer; position:relative; margin-bottom:10px;"
                     onclick="document.getElementById('imgInput').click()"
                     id="imgDropZone">
                    <div id="imgPreview" style="{{ $category->getFirstMediaUrl('image') ? '' : 'display:none;' }} margin-bottom:10px;">
                        <img id="previewImg" src="{{ $category->image_url }}" style="max-height:100px; border-radius:8px; margin:0 auto;">
                    </div>
                    <div id="imgPlaceholder" style="{{ $category->getFirstMediaUrl('image') ? 'display:none;' : '' }}">
            ...
            <div style="margin-bottom:24px; background:#111; border:1px solid #222; border-radius:8px; padding:14px 16px; display:flex; align-items:center; justify-content:space-between;">
                ...
                <div class="toggle-wrap">
                    ...
                    <label for="status" style="color:#888; font-size:13px;">{{ old('status', $category->status) ? 'Yes' : 'No' }}</label>
                </div>
            </div>
