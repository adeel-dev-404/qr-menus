...
<div style="max-width:720px;">
    ...
    <form method="POST" action="{{ route('dashboard.deals.update', $deal) }}" enctype="multipart/form-data" id="dealForm">
    @csrf
    @method('PUT')

    {{-- ===== BASIC INFO ===== --}}
    <div class="form-card">
        <p class="section-title">🎁 Edit Deal Information</p>
        <p class="section-sub">Update the details of this deal combo</p>

        <div style="margin-bottom:16px;">
            <label class="form-label">Deal Name (English) *</label>
            <input type="text" name="name" value="{{ old('name', $deal->name) }}" placeholder="e.g. Family Feast Combo"
        ...
        <div style="margin-bottom:16px;">
            ...
            <textarea name="description" rows="3" placeholder="Describe what's in the deal..."
                      class="form-input" style="resize:vertical;">{{ old('description', $deal->description) }}</textarea>
        ...
        <div class="two-col" style="margin-bottom:16px;">
            <div>
                ...
                <input type="number" name="price" value="{{ old('price', $deal->price) }}" placeholder="e.g. 1500" step="any"
            ...
            <div>
                ...
                @if($deal->image)
                    <div style="display:flex; align-items:center; gap:12px; margin-bottom:8px;">
                        <img src="{{ $deal->image_url }}" alt="Deal image" style="width:50px; height:50px; border-radius:6px; object-fit:cover; border:1px solid #2a2a2a;">
                        <span style="font-size:12px; color:#666;">Current image</span>
                    </div>
                @endif
                <input type="file" name="image" class="form-input" accept="image/*">
                <p class="form-hint">Leave empty to keep existing. JPG, PNG, WebP. Max 2MB.</p>
        ...
        <div style="background:#111; border:1px solid #222; border-radius:8px; padding:14px 16px; display:flex; align-items:center; justify-content:space-between;">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="is_available" value="1" {{ old('is_available', $deal->is_available) ? 'checked' : '' }} style="width:20px; height:20px;">
            </label>
        </div>
    </div>
...
window.addEventListener('DOMContentLoaded', () => {
    // Map existing deal items to the format matching input
    const oldItems = @json(old('deal_items', $deal->items->map(fn($item) => [
