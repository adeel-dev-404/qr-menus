<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Welcome to {{ config('app.name') }}</title>
<style>
  * { box-sizing:border-box; margin:0; padding:0; }
  body { background:#f4f4f5; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; -webkit-font-smoothing:antialiased; }
  .wrapper { max-width:580px; margin:32px auto; padding:0 16px 32px; }
  .card { background:#fff; border-radius:16px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,.06); }
  .header { background:linear-gradient(135deg,#e8502a,#c43e1c); padding:28px 32px; text-align:center; }
  .header-icon { font-size:36px; margin-bottom:8px; }
  .header h1 { color:#fff; font-size:20px; font-weight:800; letter-spacing:-.3px; margin-bottom:4px; }
  .header p  { color:rgba(255,255,255,.75); font-size:13px; }
  .body { padding:28px 32px; }
  .section-title { font-size:11px; font-weight:700; color:#9ca3af; text-transform:uppercase; letter-spacing:.08em; margin-bottom:12px; }
  .info-box { background:#f9fafb; border:1px solid #e5e7eb; border-radius:10px; overflow:hidden; margin-bottom:20px; }
  .meta-row { display:flex; justify-content:space-between; align-items:center; padding:10px 16px; border-bottom:1px solid #f3f4f6; }
  .meta-row:last-child { border-bottom:none; }
  .meta-lbl { font-size:12px; color:#9ca3af; }
  .meta-val { font-size:13px; font-weight:600; color:#374151; text-align:right; }
  .pending-box { background:#fffbeb; border:1px solid #fde68a; border-radius:10px; padding:16px; margin-bottom:20px; text-align:center; }
  .pending-box p { font-size:13px; color:#92400e; line-height:1.6; }
  .pending-box .icon { font-size:28px; margin-bottom:8px; }
  .features { margin-bottom:20px; }
  .feature { display:flex; gap:10px; margin-bottom:12px; }
  .feature-icon { font-size:18px; flex-shrink:0; }
  .feature-text { font-size:13px; color:#374151; line-height:1.5; }
  .feature-text strong { color:#111; }
  .badge-pending { display:inline-block;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:700;background:#fef3c7;color:#92400e; }
  .footer { background:#f9fafb; border-top:1px solid #e5e7eb; padding:20px 32px; text-align:center; }
  .footer p { font-size:11px; color:#9ca3af; line-height:1.6; }
  @media(max-width:600px){
    .body,.header,.footer { padding:20px; }
    .meta-row { flex-direction:column; align-items:flex-start; gap:2px; }
    .meta-val { text-align:left; }
  }
</style>
</head>
<body>
<div class="wrapper">
  <div class="card">

    <div class="header">
      <div class="header-icon">👋</div>
      <h1>Welcome to {{ config('app.name') }}!</h1>
      <p>Your restaurant registration has been received</p>
    </div>

    <div class="body">

      <p style="font-size:14px;color:#374151;line-height:1.6;margin-bottom:20px;">
        Hi <strong>{{ $user->name }}</strong>, thank you for registering <strong>{{ $restaurant->name }}</strong> on {{ config('app.name') }}! We're excited to have you on board.
      </p>

      <div class="pending-box">
        <div class="icon">⏳</div>
        <p>
          <strong>Your account is pending approval.</strong><br>
          Our team will review your registration and activate your restaurant shortly.
          You'll receive an email notification once approved.
        </p>
      </div>

      <p class="section-title">Your Registration Details</p>
      <div class="info-box">
        <div class="meta-row">
          <span class="meta-lbl">Restaurant</span>
          <span class="meta-val">{{ $restaurant->name }}</span>
        </div>
        <div class="meta-row">
          <span class="meta-lbl">Owner</span>
          <span class="meta-val">{{ $user->name }}</span>
        </div>
        <div class="meta-row">
          <span class="meta-lbl">Email</span>
          <span class="meta-val">{{ $user->email }}</span>
        </div>
        <div class="meta-row">
          <span class="meta-lbl">Status</span>
          <span class="meta-val"><span class="badge-pending">⏳ Pending Approval</span></span>
        </div>
      </div>

      <p class="section-title">What you'll get</p>
      <div class="features">
        <div class="feature">
          <span class="feature-icon">📱</span>
          <span class="feature-text"><strong>QR Code Menu</strong> — Beautiful digital menus your customers can scan and browse instantly.</span>
        </div>
        <div class="feature">
          <span class="feature-icon">🛒</span>
          <span class="feature-text"><strong>Online Ordering</strong> — Accept dine-in and takeaway orders directly from the menu.</span>
        </div>
        <div class="feature">
          <span class="feature-icon">📊</span>
          <span class="feature-text"><strong>Dashboard & Analytics</strong> — Manage everything from your personal restaurant dashboard.</span>
        </div>
        <div class="feature">
          <span class="feature-icon">🙋</span>
          <span class="feature-text"><strong>Waiter Calls</strong> — Let customers call waiters or request bills right from their phone.</span>
        </div>
      </div>

      <p style="font-size:12px;color:#9ca3af;text-align:center;line-height:1.6;">
        We'll notify you by email once your restaurant is approved.<br>
        If you have any questions, reply to this email and we'll be happy to help.
      </p>

    </div>

    <div class="footer">
      <p>This email was sent from <strong>{{ config('app.name') }}</strong><br>
      {{ config('app.url') }}</p>
    </div>

  </div>
</div>
</body>
</html>
