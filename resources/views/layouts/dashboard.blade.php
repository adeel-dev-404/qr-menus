    ...
    <aside id="sidebar">
        ...
        <div style="padding: 20px 16px 16px; border-bottom: 1px solid #1f1f1f;">
            <div style="display:flex; align-items:center; gap:10px;">
                <div
                    style="width:36px;height:36px;background:#1d4ed8;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;">
                    🍽</div>
                <div>
                    <p style="font-weight:700;font-size:15px;margin:0;color:#fff;">QR Menu</p>
                    <p
                        style="font-size:11px;color:#666;margin:0;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
        ...
        <nav style="flex:1;overflow-y:auto;padding:10px 10px;">
            ...
            <a href="{{ route('dashboard.orders.index') }}"
                ...
                Orders
                @php
                    $pendingOrders = \App\Models\Order::where('restaurant_id', auth()->user()->restaurant_id)
                        ->where('status', 'pending')
                        ->count();
                @endphp
                @if ($pendingOrders > 0)
                    <span
                        style="margin-left:auto;background:#e8502a;color:#fff;font-size:10px;font-weight:700;
            ...
            @if (auth()->user()->restaurant)
                <a href="{{ url('/r/' . auth()->user()->restaurant->slug) }}" target="_blank" class="nav-link">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        style="width:18px;height:18px;flex-shrink:0">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div style="padding:12px 10px;border-top:1px solid #1f1f1f;">
            <a href="{{ route('dashboard.profile.index') }}"
                style="display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:8px;text-decoration:none;margin-bottom:8px;transition:background .15s;"
                ...
                @else
                    <div
                        style="width:34px;height:34px;border-radius:99px;background:#1d4ed8;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;color:#fff;flex-shrink:0;">
                        {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                    </div>
                @endif
                <div style="overflow:hidden;">
                    <p
                        style="font-size:13px;font-weight:600;color:#fff;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                        {{ auth()->user()->name }}
                    </p>
            ...
            @if (session('success'))
                <div class="flash flash-success">
                    ...
                    <button onclick="this.parentElement.remove()"
                        style="background:none;border:none;cursor:pointer;color:inherit;font-size:18px;line-height:1;">&times;</button>
                </div>
            @endif
            @if (session('error'))
